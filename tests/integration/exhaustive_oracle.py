...
Two tests consume it: `test_oracle_correctness.py` validates the oracle itself
against handcrafted graphs with known-by-inspection optima, and
`test_solver_on_toy_graph.py` feeds GRASP and the oracle the same toy fixture and
gates on a quality ratio. The first exists because the second would otherwise be
validating against an unvalidated oracle (PRD Appendix A).

Semantics:

- A **route** is a directed walk in `graph.graph` obeying **undirected
  base-segment reuse** (FR5): a sequence of edges where each consecutive pair
  shares an endpoint and no non-exempt base segment is traversed more than once,
  *in either direction*. The rule keys on the `base_segment_id` tags and is
  single-sourced with the GRASP solver and the validator through
  `steeproute.solver.reuse`, so the oracle and GRASP enumerate the identical
  feasible set — which is the whole basis of the quality ratio. Exempt short
  connectors (`reusable`) may recur; node-revisits via distinct non-conflicting
  segments are allowed. Open and closed walks are both admissible; the start node
  is unconstrained.
- The **objective** is `sum(e.d_plus_m + e.d_minus_m for e in route.edges)` —
  ...
  Super-edges already carry aggregated metrics, and the oracle never re-expands
  them via `super_edge_to_base`.
- **Feasibility filters** applied during DFS:
    ...
    - Direction-aware descent cap (FR32, opt-in): a *descending* traversal whose
      ...
      cap is unset. Single-sourced with GRASP + the validator.

  Two different identities are in play and must not be conflated. The
  *candidate-dedup* key is **directed** (`(node_u, node_v, key)`) — its only job
  is to collapse different traversal orderings of the same directed edge-set. The
  *distinctness/Jaccard* step (the `TopNTracker` below) keys on the **undirected**
  `base_segment_id`, so opposite-direction reuse of one trail counts as overlap,
  matching GRASP and the validator.
  ...
  `(Σ d_plus_m + Σ d_minus_m) / Σ length_m ≥ θ`. `_dfs` emits **every
  prefix** of every feasible walk, not just maximal walks, so the candidate set
  spans all θ-clearing prefixes; GRASP recovers the best θ-clearing prefix
  of each constructed walk (`GraspSolver._best_theta_prefix`) rather than only
  checking its maximal walk. That symmetry is what makes the two feasible sets
  identical.
- **Top-N + distinctness:** the full enumeration is deduplicated by directed
  canonical edge-set, then sorted objective-descending and fed through
  `TopNTracker(n, params.j_max, base_segment_id_map(graph))` — the same
  undirected-distinctness admission semantics GRASP uses.
...
def enumerate_best(
    ...
    Returns:
        ...
        sorted_edge_ids)` rule, so comparisons against GRASP output are
        apples-to-apples.

    Complexity is exponential in the edge count — intended for hand-built test
    graphs with <= ~15 edges (a 5-node hand-graph returns well inside a second).
    Larger inputs will not return in reasonable time.
    """
    cap_rank = parse_difficulty_cap(params.difficulty_cap)
    nx_graph = graph.graph
    # Base-segment ids subject to the once-only rule, computed once. Single-sourced
    # with GRASP + the validator (`solver.reuse`) so the oracle and GRASP share one
    # feasible set.
    ...
    # Start-node pool. Under `--start-at-junction` (FR31) the oracle
    ...
    # Route-level slope floor (FR3) applied here, post-enumeration, on each
    # complete candidate — via the same `models.route_avg_gradient` the solver's
    # finalization gate uses, so the two agree bit-for-bit and neither can admit a
    # route the other rejects over float-summation order.
    ...
    # Undirected base-segment distinctness, single-sourced with GRASP + the
    # validator via `solver.reuse` so the oracle and GRASP keep one distinct set.
...
def _dfs(
    *,
    ...
    base ids is already used (`used_segments`, undirected once-only).
    ...
    dedup key is the *directed* canonical edge-set, so different traversal orders
    of the same edge-set collapse; distinctness is a separate, undirected step in
    `enumerate_best`.
    """
    if path_edges:
    ...
    for u, v, k, data in nx_graph.out_edges(current, keys=True, data=True):
        ...
        if rank is not None and rank > cap_rank:
            continue
        # Direction-aware descent cap (FR32): mirror GRASP's RCL filter via the
        # shared `solver.descent` predicate so both enumerate one feasible set.
