"""Pinned-regression goldens (Architecture §Cat 11c/11d).
...
The fixture set lives in `regression.FIXTURES`; every registered fixture is a
zero-tolerance CI gate, and `pytest.skip` / `xfail` on these tests is not a
sanctioned workaround (Architecture §Cat 11c).
"""
