...
| 8 | Climb detection (parameter-dependent: θ, min_climb_ground_length) | `pipeline/climbs.py` |
...
**CLI split (3b):** stages 1–7 are parameter-independent (depend only on area + DEM version + OSM extract date + untagged-trails-policy) — these run in `steeproute-setup` and their output is cached. Stages 8–9 depend on solver-time parameters (θ, `min_climb_ground_length`, `L_connector`) — these run in `steeproute` on every query, fast enough to not need caching. This mapping delivers Journey 2's fast-re-query behavior (changing θ hits cache for stages 1–7, re-does 8–9 only).
...
- **5a**: Single-process for v1; solver loop designed to be parallelizable later. Revisit if measurement shows 10-minute budget delivers poor objective values.
...
- The local cache directory is the **only** shared state between `steeproute` and `steeproute-setup`. No env vars, no side-channel config.
...
- All solver parameters used (`theta`, `difficulty_cap`, `l_connector`, `min_climb_ground_length`, `j_max`, `n`, `area_cap`, `untagged_policy`, `seed`, `iter_budget`, `time_budget`, `stagnation_iters`)
...
- Parallelism in the GRASP solver (Cat 5a, conditional on measurement).
