...
- Area Specification & Invocation (FR1): center/radius area input.
...
- **4c**: pickled geometry-free payload for the graph (schema v3, Story 16.3; v2's bulk coordinate arrays are read-tolerated but no longer written), JSON manifest, geojson bounds sidecar.
...
**On-disk format (4c), schema v3 — geometry is not stored (Story 16.3).** `graph.pkl` is a pickled payload dict carrying the MultiDiGraph with per-edge `geometry` dropped, and nothing else. Post-stage-5 geometry is **redundant in a prepared entry**: it is the resampled polyline in `(lon, lat)` and `vertices_resampled` holds the same vertices as `(lat, lon, elevation)` (pinned by `tests/unit/test_dem.py::test_fixture_pipeline_vertices_resampled_matches_geometry_coords`, and re-verified per-edge on all 322,939 r20 edges plus all five committed fixture entries at the v2→v3 conversion). No post-stage-5 consumer reads the attribute: query stages 6-7 and `output.render` read `vertices_resampled`, while contraction, the solver and the validator read metrics and tags. Measured on the r20 entry: `graph.pkl` 165.8 → 121.2 MB (−26.9%), entry load 3.6-4.1 → 2.4-2.6 s (−33%), query peak RSS 1.831 → 1.711 GB (−6.6%), output byte-identical.

The payload carries its **own version**, independent of the manifest's (which stays at 3 — a format change must not shift cache keys, since `cache.py` is excluded from the pipeline content hash). Legacy v2 payloads stay **readable**: their coordinate arrays are ignored rather than rebuilt, which is exactly what a v3 read does with geometry it does not have, so the graph handed to a query is content-identical across both versions and no already-prepared cache is invalidated. This is a read-only tolerance — `steeproute-setup` only ever writes v3 — and is not a compat *shim* in the §Versioned-contract-surfaces sense: nothing is translated, and the ignored bytes are the only difference.

**Tradeoffs, unchanged since v1:** pickle is Python-version-sensitive (we pin 3.13; cross-version reads fail loudly at `pickle.load`) and not human-inspectable (hence the `bounds.geojson` + `manifest.json` sidecars for diagnostics). Full columnar decomposition (Parquet) remains rejected as premature. v1 (raw pickled graph) entries surface as `CacheCorruptedError` and re-prepare once.

**v3 reverses the assumption behind Story 13.2, not its measurement.** 13.2 correctly measured that `shapely.from_ragged_array` rebuilds geometry in bulk ~20× faster than the per-object WKB parse pickling a LineString incurs (~60% of `read_entry` on a large entry; 2.5 → 1.1 s on the Chartreuse r10 entry), and it is precisely 13.2's decision to keep geometry *out of the pickled skeleton* that made v3 a small change. What was wrong was the unexamined premise that the query needs geometry rebuilt **at all**. 13.2's other finding still holds and bounds future work here: the networkx skeleton and the `vertices_resampled` list-of-tuples both measured *faster* through pickle than through any array reconstruction, so v3 does not touch them.

**Consuming write path (Story 16.2).** `write_entry(..., consume=True)` pops `geometry` straight off the caller's graph instead of `graph.copy()`-ing it first — measured 11.77 s → 6.34 s on the r20 `cache-write` stage, with identical payload content. `cli/setup.py` is the only opt-in: it owns the graph and its run summary reads only cache metadata afterwards. The copying default stands for every other caller (3a-bis). Note that `graph.pkl` **bytes** are not a stable identity for a given graph: networkx caches its lazily built `adj`/`succ`/`edges` view objects in the graph's `__dict__`, and `copy()` materializes a different subset than a freshly built graph, so those views ride along in the pickle. Equality checks on cache entries must compare *content* (nodes, ordered edges, attrs, `vertices_resampled`), which is what the Story 16.2 and 16.3 gates did.
...
- **5a**: Single-process for v1; solver loop designed to be parallelizable later. Revisit if measurement shows 10-minute budget delivers poor objective values. **Realized in Story 14.4** (`--workers N`; default bumped 1→4 2026-07-28, `spec-cli-defaults-and-setup-radius-cap.md`) — see the "Parallelism (5a)" note below.
...
- **Setup-failure hardening (2026-07-14).** `ParallelGraspFailed` now also covers failures in parallel-specific setup that happens before any worker is spawned — serializing the lean graph (`pickle.dumps`, can OOM on a large graph; plus the `solver_graph_view` rebuild when the graph is not already lean) and creating the progress queue (can hit an OS handle/semaphore limit) — routing them to the same single-process fallback as a `BrokenProcessPool`, instead of a raw uncaught traceback. Budget math / `base_segment_id_map` above that guard are shared with the single-process solver and are not parallel-specific, so failures there are left to propagate.
...
**Pure-Python loop discipline (Story 16.5).** `_construct_one` returns the same temporary `Solution` shape but does not sum an objective that `run()` immediately discards; `_best_theta_prefix` still computes the admitted prefix objective and the canonical `_route_slope_ok(prefix)` gate remains load-bearing. `TopNTracker` uses exact endpoint paths (`j_max == 0` via disjointness, with both-empty explicitly overlapping; `j_max == 1` skips overlap work), derives general union cardinality from one allocated intersection, and caches the existing directed-edge sort key on immutable held entries. None of these changes touches RNG draws, traversal/tie order, merge order, progress, convergence, fallback, or interrupt semantics. Exact full-`Solution` equality is pinned across quality seeds 11/23/37/53/71, the real fixture, single-process, and migrating paths; regression goldens were not rebaked.

Measured 2026-07-31 on the committed solver fixture: static construction 6.88 ms, 1k steady iterations with a reused context 23.01 ms, and the ordinary lazy-build `run()` 27.70 ms (the pre-change benchmark was 38.07 ms). Isolated micro-measures attributed the removed/accelerated work separately: discarded objective sum 4.13 µs per ten 12-edge walks; `j_max=0` overlap scan 33.88→4.94 µs; `j_max=1` 33.48→0.03 µs; general Jaccard 25.52→12.45 µs; cached ordering 32.98→3.28 µs. The r20 100k solver-only run was 9.56 s (review POC anchor 13.58→11.43 s). Real r20 four-worker CLI wall moved 64.08→55.38 s with byte-identical 20-file output; peak RSS after was 1.88 GB. These are measured together with the full CLI because component timings do not substitute for end-to-end wall.
...
The three public call shapes above remain valid. Internally (Story 16.1) `validate` derives the graph-wide reuse invariants — the non-exempt base-segment id set and the base-segment map — **once per call** into a private `_GraphContext` shared by every per-route and set-level check, and passes each route's already-built `RouteMetrics` into the slope-floor test rather than recomputing it. They are functions of the graph alone, so deriving them per route cost N identical full-contracted-graph scans (10 × ~327k edges at r20; `validate-render` 7.6–9.5 s → ~2.7 s measured). Story 16.5 adds one narrow optional `segment_map=` keyword to `validate`: the query parent supplies the exact map it already built for solver merge/static context, while standalone, fallback-without-state, and direct validator calls continue deriving it themselves. `validate_route` / `validate_set` retain standalone construction, and supplied-vs-derived verdict equality is pinned. The context lives in `validator.py`, **not** `models.py`: it is derived validation state rather than stage data, and `models.py` is content-hashed into every prepared cache key (same reasoning as Cat 5a's `workers` exclusion).
...
- All solver parameters used (`theta`, `min_climb_slope`, `difficulty_cap`, `l_connector`, `min_climb_ground_length`, `j_max`, `n`, `untagged_policy`, `seed`, `iter_budget`, `time_budget`, `stagnation_iters`)
