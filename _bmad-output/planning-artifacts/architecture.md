...
The three public signatures above are the contract and are unchanged. Internally (Story 16.1) `validate` derives the graph-wide reuse invariants — the non-exempt base-segment id set and the base-segment map — **once per call** into a private `_GraphContext` shared by every per-route and set-level check, and passes each route's already-built `RouteMetrics` into the slope-floor test rather than recomputing it. They are functions of the graph alone, so deriving them per route cost N identical full-contracted-graph scans (10 × ~327k edges at r20; `validate-render` 7.6–9.5 s → ~2.7 s measured). `validate_route` / `validate_set` build a context themselves when called standalone, so their verdicts are identical either way — pinned by a test asserting the two paths agree per route. The context lives in `validator.py`, **not** `models.py`: it is derived validation state rather than stage data, and `models.py` is content-hashed into every prepared cache key (same reasoning as Cat 5a's `workers` exclusion).
