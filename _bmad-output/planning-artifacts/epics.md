...
| FR3 (slope floor θ) | Epic 1 (flag) / Epic 3 (enforcement) | Flag in `cli/_shared.py`; enforced by validator + solver |
...
| FR12 (graceful degradation) | Epic 4 | Run summary messaging + distinctness-tracker output |
| FR13 (progress emission) | Epic 4 | `ProgressEvent` + throttled callback + CLI renderer |
| FR14 (Ctrl-C best-so-far) | Epic 4 | CLI try/except around `solver.run()`; `best_so_far` flush |
...
| FR22 (run summary on stdout) | Epic 4 | `cli/query.py` end-of-run block |
...
| FR30 (distinct exit codes) | Epic 1 (scaffolding) / Epic 3 (code 1) / Epic 4 (code 130) | `run_entry_point` wrapper + feature-specific returns |

**NFR coverage:**

- NFR1 (compute budget ≤10min design target): Epic 4 — time-budget termination, stagnation, progress reporting surfaces elapsed
- NFR2 (16 GB memory envelope): Epic 5 — validated during gallery generation; documented if notable
- NFR3 (Ctrl-C preserves output + cache valid): Epic 4
...
**And** `pyproject.toml` configures `pytest-cov` with `--cov-fail-under` scaffolding in place (threshold may be 0 at this point; tightens in Epic 5)
...
So that FR10, FR11, FR29 have an implementation and Epic 4 can layer progress/interrupt handling on top.

**Acceptance Criteria:**

**Given** Stories 3.1, 3.3, 3.4 are complete
**When** I implement `GraspSolver(graph, params, rng, progress_callback=None)` with `run() -> list[Solution]` driving construction (greedy-random with restricted candidate list, TopNTracker-admitted) terminating on iter-budget (time-budget + stagnation land in Epic 4), plus `best_so_far` property returning `tracker.current_top()`, plus `solver/anytime.py` with interrupt hooks (real handling wires in Epic 4)
...
**And** Epic 4 is responsible for real progress UI and interrupt handling; this epic's CLI uses a stub no-op progress callback

## Epic 4: Operational Robustness

Deliver Journeys 2 and 3. Long-running queries emit throttled progress lines; Ctrl-C preserves best-so-far top-N with an "interrupted" convergence flag and exits 130; sparse areas return fewer than N routes with a clear explanation rather than silently loosening distinctness; stagnation detection converges the solver early when no further improvement is occurring; the final run summary on stdout reports parameters, routes returned vs. N requested, validation-failure count, and wall-clock total. Turns the tool from "happy-path only" to "usable on realistic queries."

### Story 4.1: ProgressEvent + throttled callback + CLI renderer with --quiet / --verbose
...
**And** `tests/e2e/test_quiet_suppresses_progress.py` runs with `--quiet` and asserts no progress lines appear during the solver phase (final run summary from Story 4.5 remains out of scope for this story)
...
### Story 4.2: Time-budget and stagnation termination
...
### Story 4.3: Interrupt handling with best-so-far preservation (FR14)
...
### Story 4.4: Graceful degradation messaging for sparse areas (FR12)
...
**When** I update `cli/query.py::main` to detect `len(validated_set.routes) < params.n`, construct a degradation explanation string naming the observed-vs-requested counts and the J_max value, and surface the explanation in the run summary (Story 4.5) AND in each emitted report's metadata
**Then** `tests/e2e/test_degradation.py` runs `steeproute` on a real-data fixture crafted to have only 2–3 distinct routes under `--j-max 0.30` (a narrow area within the Grenoble fixture) and asserts: fewer than N reports emitted; stdout's run summary (when Story 4.5 lands) OR final output contains a line matching pattern `Only X distinct routes satisfy J_max ≤ 0.30. Returning X routes; additional candidates would exceed the overlap threshold.`; exit code is 0 (graceful degradation is a normal outcome, not an error)
...
### Story 4.5: Run summary on stdout (FR22)
...
**And** `tests/e2e/test_run_summary.py::test_degraded_path` asserts the `degradation:` line appears when fewer than N routes are returned (integrates with Story 4.4)
...
**And** `tests/e2e/test_run_summary.py::test_quiet_preserves_summary` runs with `--quiet` and asserts: no progress lines during the run (Story 4.1), but the run summary still appears on stdout at the end
...
### Story 5.1: Regression golden test harness and update-regression workflow
...
### Story 5.2: Pin 2–3 Grenoble regression fixtures with zero-tolerance CI gate
...
**Given** Story 5.1 provides the harness + schema + update workflow
...
### Story 5.3: README gallery with 3–5 pre-computed example reports
...
**Given** Stories 3.11 and 4.5 are complete
**When** I pick 3–5 Grenoble-area query regions demonstrating distinct terrain character (different from Story 5.2's regression fixtures — these are gallery regions, not test fixtures; full-size queries), run `steeproute-setup` + `steeproute` for each with a fixed seed + documented params, save the generated HTML files to `docs/examples/<region_name>/route-*.html`, capture a map-screenshot PNG + elevation-profile PNG for each region's route-1
...
**And** during gallery generation, approximate peak memory usage per query is recorded; if any region exceeds 12 GB, surface in Story 5.4's Known Limitations section as an NFR2 reality check

### Story 5.4: README Known Limitations + Quickstart sections
...
**Given** Story 5.3 populates the gallery and the full tool is feature-complete
...
- **Memory envelope (NFR2)**: validated memory behavior from Story 5.3 gallery generation — "runs comfortably on a commodity 16 GB laptop" or flagged notable usage
...
### Story 5.5: Final CI threshold tightening and Linux best-effort job
...
So that Epic 5's quality commitments are enforceable in CI and the repo can credibly claim its quality bar.

**Acceptance Criteria:**

**Given** Epics 1–4 and Stories 5.1–5.4 are complete; the full test suite passes on main
